 ...
 * {@link org.apache.hadoop.io.SequenceFile}s of docid => content. The docid is
 * set as the relative path of the document from the parent directory prepended
 * with a specified prefix. You can also specify the input encoding of the text
...
public class SequenceFilesFromDirectory extends AbstractJob {
	...
	private static final String PREFIX_ADDITION_FILTER = PrefixAdditionFilter.class
			.getName();
	...
	public static void main(String[] args) throws Exception {
		...
		System.out.println("Generate sequence files from directory done!"); 
	}
	...
	public int run(String[] args) throws Exception {
		...
		if (getOption(DefaultOptionCreator.METHOD_OPTION,
				DefaultOptionCreator.MAPREDUCE_METHOD).equals(
	...
	private int runSequential(Configuration conf, Path input, Path output,
		...
		try {
			...
			if (PrefixAdditionFilter.class.getName()
					.equals(fileFilterClassName)) {
				pathFilter = new PrefixAdditionFilter(conf, keyPrefix, options,
						writer, charset, fs);
			} else {
				pathFilter = ClassUtils.instantiateAs(fileFilterClassName,
						SequenceFilesFromDirectoryFilter.class, new Class[] {
								Configuration.class, String.class, Map.class,
								ChunkedWriter.class, Charset.class,
								FileSystem.class }, new Object[] { conf,
								keyPrefix, options, writer, charset, fs });
			}
	...
	private int runMapReduce(Path input, Path output) throws IOException,
		...
		jobConfig.set("mapreduce.job.max.split.locations",
				String.valueOf(MAX_JOB_SPLIT_LOCATIONS));
